...
SRA <- "SRR6227128"
my_blast_Genome_files <- list.files(pattern = "\\outBLAST.txt")
my_blast_Genome_names <- gsub("_contigs_outBLAST.txt","",my_blast_Genome_files)
...
got_plas_table <- read.csv("../resut_tables/SRR6227128_got_plamid.csv")
#============
...
#--------- set column name for genome_match
...
which(uniq_genome_short %in% my_gff)
...
for(i in which(uniq_genome_short %in% my_gff)){
  ...
  for(j in 1:nrow(ol.gene)){
    my_gen$locus[ol.gene$queryHits[j]] <- as.character(my_gff_table$GenBank_locus_tag[ol.gene$subjectHits[j]])
    my_gen$gene[ol.gene$queryHits[j]] <- as.character(my_gff_table$Gene_name[ol.gene$subjectHits[j]])
    my_gen$product[ol.gene$queryHits[j]] <- as.character(my_gff_table$Product[ol.gene$subjectHits[j]])
    my_gen$ref_gene_size[ol.gene$queryHits[j]] <- as.character(my_gff_table$Locus_size[ol.gene$subjectHits[j]])
