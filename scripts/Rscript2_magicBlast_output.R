...
ref <- readFastaRef(paste("../plasmid_fasta/", my_magic_plas_name, ".fasta", sep = ""))
lgth_ref <- length(ref)  
...
output_list[[o]]$overlapping_contigs <- NULL

#--------- create contig.fasta file
outfile <- paste("../contig_fasta/", my_magic_plas_name,"_matching_contigs.fasta", sep = "")
for(i in 1: nrow(coord)){
...
#--------- Save the table
write.csv(m,paste("../resut_tables/",SRA,"_got_plamid.csv", sep = ""), row.names = F)
#==========================================================================================================================================
