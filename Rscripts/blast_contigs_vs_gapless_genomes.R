...
contigs_fasta <- list.files(pattern = "\\contigs.fasta")
...
for (i in 1: length(contigs_fasta)) {
  cat(paste("blastn -db ../../gapless_genomes/fasta/gapless_genomes.fasta -num_threads 24 -max_target_seqs 5 -outfmt '7 std sgi stitle' -evalue 0.0000000001 -query ", contigs_fasta[i]," -out ../blast_vs_genome_output/", contigs_blast[i], sep = ""),sep="\n",file=Script,append=TRUE)
}
